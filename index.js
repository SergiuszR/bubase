...
const hueRange = 0.15 + Math.random()  *0.2; // Reduced range for subtler gradients
const saturation = 0.6 + Math.random()*  0.2; // Reduced variation
const brightnessMin = 0.4 + Math.random()  *0.1; // Narrower brightness range
...
if (emailForm && submitButton && emailInput) {
    ...
    // IMPORTANT: Show the form again (Webflow hides it after submission)
    emailForm.style.display = '';
    emailForm.style.visibility = '';
    emailForm.style.opacity = '';
    
    // Also ensure the form container is visible
    ...
    // Reset form submission flags
    thanksAnimationTriggered = false;
    formSubmissionInProgress = false;
    
    console.log('✅ Form reset complete and made visible');
} else {
    console.log('⚠️ Could not find form elements to reset');
}
...
console.log('Setting up Webflow form detection...');
...
console.log('Initial button disabled state:', buttonWasDisabled);

// Method 1: Watch for button becoming enabled
const buttonObserver = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
        if (mutation.type === 'attributes' && mutation.attributeName === 'disabled') {
            const isNowDisabled = submitButton.hasAttribute('disabled');
            
            if (buttonWasDisabled && !isNowDisabled) {
                console.log('✅ Submit button became enabled - user can now submit');
            } else if (!buttonWasDisabled && isNowDisabled) {
                console.log('🔄 Submit button became disabled again');
            }
...
// Method 2: Listen for actual form submission


// Method 3: Listen for button clicks (backup)
submitButton.addEventListener('click', (e) => {
    console.log('🖱️ Submit button clicked');
    console.log('Button disabled:', submitButton.hasAttribute('disabled'));
    console.log('Thanks animation already triggered:', thanksAnimationTriggered);
    
    if (!submitButton.hasAttribute('disabled') && !formSubmissionInProgress && !thanksAnimationTriggered) {
        console.log('Valid button click detected, starting monitoring...');
...
// Method 4: Monitor for success message (even though it's hidden)
function monitorForSuccessMessage() {
    console.log('🔍 Starting success message monitoring...');
    let attempts = 0;
    const maxAttempts = 50; // 5 seconds
    
    const checkForSuccess = () => {
        if (thanksAnimationTriggered) {
            console.log('Thanks animation already triggered, stopping monitoring');
            return;
        }
        ...
        if (successDiv) {
            // Check if Webflow would have shown it (even though we hide it)
            const hasSuccessContent = successDiv.textContent.trim().length > 0;
            const webflowTriedToShow = successDiv.style.display === 'block';
            
            console.log(`Success check ${attempts + 1}/50:`, {
                hasContent: hasSuccessContent,
                webflowTriedToShow: webflowTriedToShow,
                textContent: successDiv.textContent.trim()
            });
            
            if (hasSuccessContent && webflowTriedToShow) {
                console.log('🎉 SUCCESS DETECTED! Triggering Thanks animation!');
                formSubmissionInProgress = false;
                handleFormSuccess();
                return;
            }
        ...
        if (attempts < maxAttempts) {
            ...
            console.log('⏰ Success monitoring timed out');
            formSubmissionInProgress = false;
            
            // Fallback - if we detected a submission but no success message, still trigger
            console.log('🔄 Triggering fallback success animation');
            handleFormSuccess();
        }
    };
...
// Method 5: Manual trigger for testing
window.triggerThanks = () => {
    console.log('🧪 Manual trigger activated');
    if (!thanksAnimationTriggered) {
        handleFormSuccess();
    } else {
        console.log('Thanks animation already triggered, ignoring manual trigger');
    }
};

console.log('✅ Webflow form detection setup complete');
...
console.log('Form success already handled, ignoring duplicate call');
return;
}
...
formSubmissionInProgress = false;
console.log('🎉 FORM SUCCESS - Showing Thanks animation!');

// Change to celebratory colors
currentTheme = {
    hueRange: [0.3, 0.5], // Green to cyan - more subtle range
    saturation: 0.7,
...
// Reset after 5 seconds - including form reset
setTimeout(() => {
    console.log('🔄 Resetting sphere to normal state');
    isFormSubmitted = false;
    morphToCircle();
    
    // Reset form after sphere animation starts
    setTimeout(() => {
        resetFormToInitialState();
    }, 500); // Small delay to let sphere animation start
}, 5000);
...
function init() {
// Skip in Webflow editor
if (window.Webflow && window.Webflow.env && window.Webflow.env() === 'editor') {
console.log('Webflow editor detected - sphere disabled');
return;
}


// Hide Webflow success message first
...
// Setup form detection after DOM is ready
setTimeout(() => {
...
testButton.innerHTML = '🧪 Test Thanks';
testButton.style.cssText = colorButton.style.cssText;

testButton.addEventListener('click', () => {
    console.log('🧪 Test button clicked - triggering Thanks');
...
testButton.addEventListener('mouseenter', () => {
    testButton.style.background = 'rgba(100,255,100,0.2)';
    testButton.style.borderColor = 'rgba(100,255,100,0.6)';
    testButton.style.transform = 'translateY(-2px)';
    testButton.style.boxShadow = '0 6px 20px rgba(100,255,100,0.3)';
});

testButton.addEventListener('mouseleave', () => {
    testButton.style.background = 'rgba(0,0,0,0.5)';
    testButton.style.borderColor = 'rgba(255,255,255,0.3)';
    testButton.style.transform = 'translateY(0px)';
    testButton.style.boxShadow = '0 4px 15px rgba(0,0,0,0.2)';
});
...
// Much more subtle gradient calculation
const angle = Math.atan2(point.y, point.x);
const normalizedAngle = (angle + Math.PI) / (2 * Math.PI);

// Gentle time-based animation
const timeOffset = colorTime * 0.1; // Much slower

// Smooth gradient mixing - much more subtle
const depthInfluence = depth * 0.3; // Reduced influence
const angleInfluence = normalizedAngle * 0.2; // Reduced influence
const timeInfluence = (Math.sin(timeOffset + index * 0.001) + 1) * 0.05; // Very subtle
...
if (isExploding && explosionParticles[index]) {
    ...
    // Smooth gradient across the sphere
    hue = currentTheme.hueRange[0] + gradientMix * hueRange;
    
    // Very subtle brightness variation
    const brightnessVariation = (Math.sin(timeOffset * 0.5 + depth * 2) + 1) * 0.03; // Much smaller variation
    const brightness = currentTheme.brightnessRange[0] + 
                      gradientMix * (currentTheme.brightnessRange[1] - currentTheme.brightnessRange[0]) +
                      brightnessVariation;
    
    // Minimal saturation variation
    const saturationVariation = (Math.sin(timeOffset * 0.3 + normalizedAngle * Math.PI) + 1) * 0.02; // Very small
...
// NO INPUT EVENT LISTENERS - sphere won't change when typing
console.log('Event listeners setup - input morphing disabled');
}
...
if (emailForm) {
    emailForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (result.success) {
                // Trigger your existing success handler
                handleFormSuccess();
            } else {
